import React, { useState } from 'react'; import { useLanguage } from
'../LanguageContext.tsx'; import { motion, AnimatePresence } from
'motion/react'; import { Heart, Calendar, Award, ShieldAlert, Clock,
AlertTriangle, Settings, Check, Bell, RefreshCw, X, Plus, ExternalLink, Home,
User, Link, Pause, MapPin, Download, Mail, MessageSquare, Inbox, Eye, EyeOff }
from 'lucide-react'; import { Donor, DonorCenter, Donation, MedicalNote,
BloodCenter, formatBloodGroup, formatRhFactor, getGamificationStatus } from
'../types'; import { calculateNextDates } from '../utils/intervals'; import {
ConfirmationModal } from './ConfirmationModal'; import { ResponsiveContainer,
AreaChart, Area, XAxis, YAxis, Tooltip, CartesianGrid } from 'recharts';

interface DonorSectionProps { donor: Donor; links: DonorCenter[]; donations:
Donation[]; medicalNotes: MedicalNote[]; readiness: { ready: boolean; reason?:
string; pendingConfirmation?: boolean }; centers: BloodCenter[]; onRefresh: ()
=> void; apiBase: string; token: string; }

export default function DonorSection({ donor, links, donations, medicalNotes,
readiness, centers, onRefresh, apiBase, token }: DonorSectionProps) { const { t
} = useLanguage();

const [activeMenu, setActiveMenu] = useState<'dashboard' | 'profile' | 'history'
| 'links' | 'pause' | 'notifications' | 'account'>('dashboard'); const
[refreshing, setRefreshing] = useState(false); const [isSaving, setIsSaving] =
useState(false);

// Set local pause states const [pauseForm, setPauseForm] = useState({
personalPause: donor.personalPause, personalPauseUntil: donor.personalPauseUntil
|| '', personalPauseNote: donor.personalPauseNote || '' }); const [pauseSuccess,
setPauseSuccess] = useState('');

// Notifications toggles const [notifForm, setNotifForm] = useState({
pushEnabled: donor.pushEnabled, emailNotificationsEnabled:
donor.emailNotificationsEnabled }); const [notifSuccess, setNotifSuccess] =
useState('');

// Received notifications list states const [notificationsHistory,
setNotificationsHistory] = useState<any[]>([]); const [loadingNotifications,
setLoadingNotifications] = useState(false); const unreadCount =
notificationsHistory.filter(n => !n.isRead).length;

const [confirmConfig, setConfirmConfig] = useState<{ isOpen: boolean; title:
string; message: string; confirmText?: string; cancelText?: string; variant?:
'danger' | 'success' | 'warning' | 'info'; onConfirm: () => void; }>({ isOpen:
false, title: '', message: '', onConfirm: () => {} });
...
const fetchNotificationsHistory = async () => { try {
setLoadingNotifications(true); const res = await
fetch(${apiBase}/donor/notifications/${donor.id}, { headers: { 'Authorization':
token } }); const data = await res.json(); if (res.ok && data.success) {
setNotificationsHistory(data.notifications || []); } } catch (e) {
console.error('Error fetching notifications history', e); } finally {
setLoadingNotifications(false); } };

const markAllAsRead = async () => { if (unreadCount === 0) return; try { await
fetch(${apiBase}/donor/notifications/read-all, { method: 'POST', headers: {
'Content-Type': 'application/json', 'Authorization': token }, body:
JSON.stringify({ donorId: donor.id }) }); // Update local state
setNotificationsHistory(prev => prev.map(n => ({ ...n, isRead: true }))); }
catch (e) { console.error('Error marking as read', e); } };

React.useEffect(() => { fetchNotificationsHistory(); }, [donor.id]);

React.useEffect(() => { if (activeMenu === 'notifications') { markAllAsRead(); }
}, [activeMenu]);

// Additional link center state const [selectedCenterId, setSelectedCenterId] =
useState(''); const [linkError, setLinkError] = useState(''); const
[linkSuccess, setLinkSuccess] = useState('');

// Profile edit states const [isEditingProfile, setIsEditingProfile] =
useState(false);

// Security password states const [showCurrentPassword, setShowCurrentPassword]
= useState(false); const [showNewPassword, setShowNewPassword] =
useState(false); const [showRepeatPassword, setShowRepeatPassword] =
useState(false); const [currentPassword, setCurrentPassword] = useState('');
const [newPassword, setNewPassword] = useState(''); const [repeatPassword,
setRepeatPassword] = useState(''); const [passwordError, setPasswordError] =
useState(''); const [passwordSuccess, setPasswordSuccess] = useState('');

const getNewPasswordErrorStr = (val: string): string => { if (!val) return '';
if (val.length < 6) return t('Пароль должен быть не менее 6 символов'); const
hasLetter = /[a-zA-Zа-яА-ЯёЁіІўЎ]/.test(val); const hasDigit = /\d/.test(val);
if (!hasLetter || !hasDigit) { return t('пароль не надёжный должны
присутствовать цифры и буквы'); } return ''; };

const getRepeatPasswordErrorStr = (val: string): string => { if (!val) return
''; if (val !== newPassword) { return t('Неверный пароль'); } return ''; };

const handlePasswordSubmit = (e: React.FormEvent) => { e.preventDefault();
...
const formatBelarusPhone = (val: string): string => { if (!val) return '+375';
const digits = val.replace(/\D/g, ''); if (digits.startsWith('375')) { const
remaining = digits.slice(3, 12); return '+375' + remaining; } else { return
'+375' + digits.slice(0, 9); } };

const handlePhoneChange = (e: React.ChangeEvent) => { let val = e.target.value;
if (!val.startsWith('+375')) { const digitsOnly = val.replace(/\D/g, ''); if
(digitsOnly.startsWith('375')) { val = '+375' + digitsOnly.slice(3, 12); } else
{ val = '+375'; } } else { const afterPrefix = val.slice(4).replace(/\D/g,
'').slice(0, 9); val = '+375' + afterPrefix; } setProfileForm(prev => ({
...prev, phone: val })); };

const handleNameChange = (field: 'lastName' | 'firstName' | 'middleName', val:
string) => { const lettersOnly = val.replace(/[^a-zA-Zа-яА-ЯёЁіІўЎ\-]/g, '');
setProfileForm(prev => ({ ...prev, [field]: lettersOnly })); };

const handleCancelEdit = () => { setProfileForm({ lastName: donor.lastName,
firstName: donor.firstName, middleName: donor.middleName || '', birthDate:
donor.birthDate ? donor.birthDate.split('T')[0] : '', gender: donor.gender,
phone: formatBelarusPhone(donor.phone), email: donor.email || '', weight:
donor.weight, bloodGroup: donor.bloodGroup, rhFactor: donor.rhFactor });
setEditError(''); setEditSuccess(''); setIsEditingProfile(false); };

const handleStartEdit = () => { setProfileForm({ lastName: donor.lastName,
firstName: donor.firstName, middleName: donor.middleName || '', birthDate:
donor.birthDate ? donor.birthDate.split('T')[0] : '', gender: donor.gender,
phone: formatBelarusPhone(donor.phone), email: donor.email || '', weight:
donor.weight, bloodGroup: donor.bloodGroup, rhFactor: donor.rhFactor });
setEditError(''); setEditSuccess(''); setIsEditingProfile(true); };

const [profileForm, setProfileForm] = useState({ lastName: donor.lastName,
firstName: donor.firstName, middleName: donor.middleName || '', birthDate:
donor.birthDate ? donor.birthDate.split('T')[0] : '', gender: donor.gender,
phone: formatBelarusPhone(donor.phone), email: donor.email || '', weight:
donor.weight, bloodGroup: donor.bloodGroup, rhFactor: donor.rhFactor }); const
[editSuccess, setEditSuccess] = useState(''); const [editError, setEditError] =
useState('');

const [showAppointmentModal, setShowAppointmentModal] = useState(false); const
[appointmentForm, setAppointmentForm] = useState({ centerId: 0, appointmentDate:
'', appointmentTime: '10:00', donationType: 'blood' }); const
[appointmentSuccess, setAppointmentSuccess] = useState('');

React.useEffect(() => { setProfileForm({ lastName: donor.lastName, firstName:
donor.firstName, middleName: donor.middleName || '', birthDate: donor.birthDate
? donor.birthDate.split('T')[0] : '', gender: donor.gender, phone:
formatBelarusPhone(donor.phone), email: donor.email || '', weight: donor.weight,
bloodGroup: donor.bloodGroup, rhFactor: donor.rhFactor }); }, [donor.id,
donor.lastName, donor.firstName, donor.middleName, donor.birthDate,
donor.gender, donor.phone, donor.email, donor.weight, donor.bloodGroup,
donor.rhFactor]);

const handleProfileSubmit = (e: React.FormEvent) => { e.preventDefault();
setEditSuccess(''); setEditError(''); if (profileForm.phone.length !== 13) {
setEditError(t('Номер телефона должен содержать ровно 13 символов (например,
+375XXXXXXXXX)')); return; } const emailRegex = /^[^\s@]+@[^\s@]+$/; if
(!emailRegex.test(profileForm.email)) { setEditError(t('E-mail должен быть в
формате имя@домен (например, donor@example.com)')); return; }
...
const triggerRefresh = async () => { setRefreshing(true); await onRefresh();
setRefreshing(false); };

// Submit pause changes const handlePauseSubmit = (e: React.FormEvent) => {
e.preventDefault(); setPauseSuccess(''); requestConfirm({ title: t('Сохранить
настройки паузы?'), message: t('Вы уверены, что хотите обновить статус вашей
паузы? Во время активной паузы вы не будете получать приглашения на донацию.'),
variant: 'warning', confirmText: t('Сохранить'), onConfirm: async () => {
setIsSaving(true); try { const res = await fetch(${apiBase}/donor/pause, {
method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization':
token }, body: JSON.stringify({ donorId: donor.id, ...pauseForm }) }); if
(res.ok) { setPauseSuccess(t('Настройки личной паузы успешно сохранены!'));
onRefresh(); } } catch { setPauseSuccess(t('Не удалось сохранить изменения во
внешнем сервисе')); } closeConfirm(); } }); };

// Submit notifications settings const handleNotifSubmit = (e: React.FormEvent)
=> { e.preventDefault(); setNotifSuccess(''); requestConfirm({ title:
t('Сохранить настройки уведомлений?'), message: t('Текущие каналы для связи
будут обновлены. Вы уверены?'), variant: 'info', confirmText: t('Сохранить'),
onConfirm: async () => { setIsSaving(true); try { const res = await
fetch(${apiBase}/donor/notifications, { method: 'PUT', headers: {
'Content-Type': 'application/json', 'Authorization': token }, body:
JSON.stringify({ donorId: donor.id, ...notifForm }) }); if (res.ok) {
setNotifSuccess(t('Предпочтения каналов рассылок изменены!')); onRefresh(); } }
catch { setNotifSuccess(t('Ошибка сохранения настроек')); } closeConfirm(); }
}); };

// Resubmit a rejected center application const handleAppointmentSubmit = async
(e: React.FormEvent) => { e.preventDefault(); if (!appointmentForm.centerId ||
!appointmentForm.appointmentDate) return;
...
const handleResubmit = (centerId: number) => { requestConfirm({ title:
t('Переподать заявку?'), message: t('Вы уверены, что хотите снова отправить
заявку на прикрепление к этому центру крови?'), variant: 'info', confirmText:
t('Отправить'), onConfirm: async () => { setIsSaving(true); try { const res =
await fetch(${apiBase}/donor/resubmit/${centerId}, { method: 'POST', headers: {
'Content-Type': 'application/json', 'Authorization': token }, body:
JSON.stringify({ donorId: donor.id }) }); if (res.ok) { // alert('Заявка успешно
переподана в центр крови! Статус изменен на Ожидание.'); onRefresh(); } } catch
(err) { // alert('Ошибка при повторной отправке'); } closeConfirm(); } }); };

// Set center as primary (home) const handleSetPrimary = (centerId: number) => {
requestConfirm({ title: t('Сделать домашним центром?'), message: t('Вы хотите
установить этот центр крови как основной (домашний) для вашей донорской
активности?'), variant: 'info', confirmText: t('Установить'), onConfirm: async
() => { setIsSaving(true); try { const res = await
fetch(${apiBase}/donor/set-primary-center, { method: 'POST', headers: {
'Content-Type': 'application/json', 'Authorization': token }, body:
JSON.stringify({ donorId: donor.id, centerId }) }); if (res.ok) { onRefresh(); }
else { const data = await res.json(); } } catch (err) {} closeConfirm(); } });
};

// Send secondary link application const handleCenterLink = (e: React.FormEvent)
=> { e.preventDefault(); setLinkError(''); setLinkSuccess(''); if
(!selectedCenterId) { setLinkError(t('Пожалуйста, укажите медицинский центр'));
return; }
...
// Calculate stats const calcAge = (birthDateString: string) => { const today =
new Date(); const birthDate = new Date(birthDateString); let age =
today.getFullYear() - birthDate.getFullYear(); const m = today.getMonth() -
birthDate.getMonth(); if (m < 0 || (m === 0 && today.getDate() <
birthDate.getDate())) { age--; } return age; };

const bCounts = donor.bloodDonationsCount || 0; const pCounts =
donor.plasmaDonationsCount || 0; const plCounts = donor.plateletsDonationsCount
|| 0; const grCounts = donations.filter(d => (d.donationType || d.type) ===
'granulocytes').length; const totalDonations = donor.donationsCount || 0;

const bloodFree = donor.bloodFreeCount || 0; const compFree =
donor.compFreeCount || 0; const bloodPaid = donor.bloodPaidCount || 0; const
compPaid = donor.compPaidCount || 0;

const totalFreeDonations = bloodFree + compFree; const totalPaidDonations =
bloodPaid + compPaid;

const gameStatus = getGamificationStatus(bloodFree, compFree, bloodPaid,
compPaid);

const last12MonthsData = React.useMemo(() => { const months = [t('Янв'),
t('Фев'), t('Мар'), t('Апр'), t('Май'), t('Июн'), t('Июл'), t('Авг'), t('Сен'),
t('Окт'), t('Ноя'), t('Дек')]; const result: { name: string; count: number;
volume: number }[] = [];
...
const homeCenter = centers.find(c => { const primaryLink = links.find(l =>
l.donorId === donor.id && l.isPrimary); return c.id === primaryLink?.centerId;
});

let nextBloodDate = null; let nextAferesisDate = null; let nextGranDate = null;
let bloodDays = 60, aferezisDays = 14, granulocytesDays = 30;

if (donor.lastDonationDate && donor.lastDonationType) { const datesInfo =
calculateNextDates(donor.lastDonationType, bCounts, new
Date(donor.lastDonationDate)); nextBloodDate = datesInfo.nextBloodDate;
nextAferesisDate = datesInfo.nextAferesisDate; nextGranDate =
datesInfo.nextGranulocytesDate;
...
let recoveryProgress = 100; let recoveryDaysPassed = 0; let recoveryDaysTotal
= 0; let recoveryDaysLeft = 0;

if (!readiness.ready) { if (donor.lastDonationDate && donor.nextAvailableDate) {
const lastDate = new Date(donor.lastDonationDate).getTime(); const nextDate =
new Date(donor.nextAvailableDate).getTime(); const today = new Date().getTime();
...
return (  {/* Side Profile Card & Inner Panel Menu /}   {/ Avatar simulation
icon */}  {donor.firstName[0]}{donor.lastName[0]}   {t(donor.lastName)}
{t(donor.firstName)}  {t("Донор с")} {new
Date(donor.createdAt).toLocaleDateString('ru-RU')}
          ...
          <table className="w-full text-left text-sm border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 text-slate-600 bg-white">
              {donations.map(don => {
                ...
                const typeLabel = donType === 'blood' ? t('Цельная кровь') : donType === 'plasma' ? t('Плазма') : donType === 'platelets' ? t('Тромбоциты') : t('Гранулоциты');
