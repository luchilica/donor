...
const loops = [
  ...
  { list: 'smsTemplates', idVar: 'm' } // note: smsTemplates loop uses m
];
...
content = content.replace(/await Promise\.all\(state\.users\.map\(async user => \{\s*const prev = .*?;\s*if \(prev && JSON\.stringify\(prev\) === JSON\.stringify\(user\)\) return;\s*return prisma\.user\.upsert/g, 
  `await Promise.all(state.users.map(async user => {\n        const prev = cachedDb && cachedDb.users && cachedDb.users.find(x => x.id === user.id);\n        if (prev && JSON.stringify(prev) === JSON.stringify(user)) return;\n        return prisma.user.upsert`);
